...
비교 도구는 입력을 PostgreSQL에 적재하고 일부 셸은 결과를 DB에 쓴다. **반드시 스키마를 적용**해야 한다.

**(A) 우분투 네이티브** — 설치부터 검증까지는 **[docs/SETUP.md](docs/SETUP.md)** 를 그대로 따른다(스키마 적용 포함).

**(B) Docker로 빠르게**:

```bash
docker run -d --name dc-pg \
  -e POSTGRES_PASSWORD=devpw -e POSTGRES_DB=compare_proto \
  -p 5432:5432 postgres:16

# 스키마 + 시드(고객 20 / 거래 50) 적용
docker exec -i dc-pg psql -v ON_ERROR_STOP=1 -U postgres -d compare_proto < db/schema.sql
```
...
export POSTGRES_PASSWORD=devpw          # 비밀번호는 코드/설정이 아니라 환경변수로
```
...
RUN_DB_TESTS=1 PGHOST=localhost PGPORT=5432 PGDATABASE=compare_proto \
  PGUSER=postgres POSTGRES_PASSWORD=devpw pytest
```
