...
> ## ⚠️ 정렬 노트 (2026-06-03, 회의 후) — 반드시 먼저 읽기
>
> 본 SPEC 본문은 **프로토 기준(셸당 입력 1·출력 1, 4사분면)**으로 쓰였다. 회의로 방향이 확장됐다:
> - **목표 규격의 정본은 [DEFINITION_SPEC.md](DEFINITION_SPEC.md)** — 셸당 **다중 입력(`inputs[]`)·다중 출력(`outputs[]`)**, DB접속 공통(config), 디렉토리 config, `expected` 명시, SAM은 파일출력만(바이트 비교라 포맷터 불요), **출력 단위 집계**, 화면 모니터링 + CSV 리포트.
> - **현 구현 상태**: **다중 입력 = 완료(P1, D-033)** / **다중 출력 = 미구현(P2 예정)**. 따라서 아래 본문의 "input_type/output_type 단일·4사분면" 서술은 **P1까지의 현 동작**이며, **목표는 DEFINITION_SPEC**이다.
> - 충돌 시 **DEFINITION_SPEC.md(정본) > 이 문서**. 결정 이력은 **D-033**.
