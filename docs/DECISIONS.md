...
## D-060. Windows 지원 — mock/데모 .py화 + runner .py 인터프리터 실행 + 양 OS CI (코어 예외 승인)

**배경**: Windows에서 테스트·데모를 돌리는 팀원 존재. 막던 건 엔진이 아니라 스크립트/데모 층
(진입 스크립트·mock 배치가 `.sh`, Windows는 shebang 미지원). 파이썬 코어는 이미 크로스플랫폼(POSIX 모듈 0).
**운영 타깃은 여전히 리눅스(Net COBOL)** — 이 작업은 "팀 테스트·데모의 Windows 동작"까지(운영 배포 지원 아님).

**결정**:
1. **runner.py(★코어 예외, 사용자 승인)**: `_exec_argv()` — shell_program이 `.py`면 `[sys.executable, path]`,
   그 외(실 배치 실행파일·셸)는 기존대로 직접 호출. **실 배치 호출 계약 불변**(비-.py 경로 0 변경).
2. **데모 mock 23개 `.sh`→`.py`**(생성기 make_complete_data + 정본 complete_sample.csv 참조 + 정의 재생성).
   복사/멀티복사/MISSING/DB래퍼 전부 순수 Python.
3. **run_demo `.sh`→`run_demo.py`**(psql→psycopg2, 단계 실행 runpy in-process). `.sh`는 위임 래퍼로 잔존.
4. **GitHub Actions CI**: ubuntu+windows × py3.10/3.12 매트릭스로 매 푸시 pytest — 양 OS 동작을 자동 증명
   (Phase B의 CI 항목 앞당김). DB 통합은 RUN_DB_TESTS 게이트라 CI에선 자동 skip.
5. 문서: TEAM_SETUP/README에 Windows 명령 병기(`py`·`$env:`), "WSL 필수" 정정.

**범위/검증**: 비교 판정·정의 스키마·실행 흐름 무변경. dc-pg e2e(`python3 run_demo.py`) 27/OK22·NG4·MISSING1 불변.
310 passed(+1 runner). **남은 확인**: Windows 러너 첫 CI 통과(맥에서 단정 불가 → CI가 증명) + Windows 팀원 1명 GUI 체크리스트 1회.

## D-061. Windows CI 그린 증명 — 실패 12건 전수 수정(테스트 POSIX 가정) + 정의저장 LF

**배경**: D-060 Windows 지원 후, "Windows에서 진짜 도는가"를 CI로 증명(맥에선 단정 불가).
첫 Windows 러너에서 실패 다수 → junit 리포터(action-junit-report)를 CI에 넣어 **per-test 실패를
annotations API로 가시화**(로그 다운로드는 admin 토큰 필요해 막힘) → 정확 진단·수정.

**확정 결함(누적 12건, 거의 전부 테스트 픽스처의 POSIX 가정 — 엔진은 크로스플랫폼)**:
- test_preflight: `os.getuid()`가 Windows 미존재(수집 AttributeError) + chmod 권한테스트 → win32 skip.
- test_paths ×3 / test_settings ×3: 하드코딩 `/mnt`·`/abs`·`/g` 절대경로가 Windows서 드라이브 붙음·
  백슬래시 → tmp_path/`_osabs()` 기반 OS-절대경로, `str(Path)` 비교(구분자 무관).
- test_runner argv ×2: `endswith("a/b.py")`·`str(Path("/x"))` 구분자 의존 → basename·os.path.join.
- test_runner 실행 ×2: `.sh` 직접 실행 = WinError 193 → `.py` 픽스처(새 인터프리터 경로도 검증).
- **유일한 실코드(인터페이스)**: web.py 정의저장을 `open(newline="\n")`으로 — Windows text모드 `\n→\r\n`
  변환이 D-058 CRLF 정규화를 무효화하던 것 차단(write_text의 newline 인자는 3.10+라 open 사용=3.9 호환).

**결과**: CI 매트릭스(ubuntu+windows × py3.10/3.12) **4잡 전부 green**(커밋 4fbb4a9). 비교 판정·코어 무변경.
맥/CI 공통 310 passed/10 skipped. → main 머지.
