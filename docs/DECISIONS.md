...
5. **빈 양식 배포(규격 제공)**: 고객이 "뭘 적을지" 고민 않게 **매핑표 템플릿**(`MAPPING_TEMPLATE_CSV` = 필수열 + 기입 예시 2행)을 `/definition/mapping-template`(GET, BOM 부착 — Excel 깨짐 방지)로 다운로드 제공. 받아서 값만 채워 올리면 정의 자동 생성. 템플릿 자체가 유효 매핑표라 그대로 올려도 동작.
