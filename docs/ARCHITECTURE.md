...
> ## ⚠️ 정렬 노트 (2026-06-03, 회의 후)
>
> 흐름도·모듈 책임은 **프로토(셸당 입력 1·출력 1)** 기준이다. 회의로 **다중 입력·다중 출력**으로 확장 중:
> - **Load**: 셸당 `inputs[]`를 **루프 적재**(여러 테이블/파일) — 코어 `load_input_csv` 재사용. **완료(P1)**.
> - **Run**: `program`(잡) **1회 호출**(내부 프로그램 여러 개·여러 DB 참조는 잡 내부 = 블랙박스, 도구 무관). 변경 없음.
> - **Download/Compare**: 셸당 `outputs[]`를 **루프**로 추출(DB→CSV export / file→그대로)·각 `expected`와 바이트 비교 → **셸당 결과 N건(출력 단위)**. **미구현(P2)**.
> - **코어 무재작성**: loader/runner/exporter/comparator는 "1건" 범용이라 **리스트 루프로 감쌀 뿐**.
> - 정본 규격 = [DEFINITION_SPEC.md](DEFINITION_SPEC.md), 결정 = D-033. 아래 본문의 단일 입출력 다이어그램은 P1까지의 현 동작.
